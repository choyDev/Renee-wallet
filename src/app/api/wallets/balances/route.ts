...
const USDT_ADDR = {
  ETH: CHAIN_ENV === "mainnet" ? process.env.USDT_ETH_MAINNET : process.env.USDT_ETH_TESTNET,
  TRX: CHAIN_ENV === "mainnet" ? process.env.USDT_TRX_MAINNET : process.env.USDT_TRX_TESTNET,
  SOL: CHAIN_ENV === "mainnet" ? process.env.USDT_SOL_MAINNET : process.env.USDT_SOL_TESTNET,
} as const;
...
// ───────────────────────────────────────────────────────────────────────────────
// 🚀 NEW: Timeout wrapper for any promise
// ───────────────────────────────────────────────────────────────────────────────
...
async function getUsdPrices(symbols: ("SOL" | "TRX" | "ETH" | "BTC" | "DOGE" | "XRP" | "XMR")[]): Promise<Prices> {
  ...
  const out: Prices = { SOL: 0, TRX: 0, ETH: 0, BTC: 0, DOGE: 0, XRP: 0, XMR: 0, USDT: 1 };
  await Promise.all(
    (Object.keys(syms) as Array<keyof typeof syms>).map(async (k) => {
      try {
        const r = await fetch(`https://api.binance.com/api/v3/ticker/price?symbol=${syms[k]}`, { cache: "no-store" });
        const j = await r.json();
        out[k] = j?.price ? Number(j.price) : 0;
...
async function getDogeBalance(address: string) {
  const base = CHAIN_ENV === "testnet" ? "https://doge-electrs-testnet-demo.qed.me" : "https://dogechain.info/api";
  try {
    const res = await fetch(`${base}/address/${address}`, { cache: "no-store" });
    if (!res.ok) throw new Error(`Fetch failed ${res.status}`);
    const data = await res.json();
...
async function getXrpBalance(address: string) {
  const rpc = CHAIN_ENV === "testnet" ? "wss://s.altnet.rippletest.net:51233" : "wss://xrplcluster.com";
  const client = new Client(rpc);
  await client.connect();
  const res = await client.request({ command: "account_info", account: address });
  await client.disconnect();
...
async function getTrc20Balance(rpcUrl: string, ownerBase58: string, tokenBase58: string) {
  ...
  const [raw, dec] = await Promise.all([c.balanceOf(ownerBase58).call(), c.decimals().call().catch(() => 6)]);
  const rawStr = typeof raw === "string" ? raw : raw?.toString?.() ?? "0";
  return Number(rawStr) / 10 ** Number(dec);
}
...
export async function GET(req: Request) {
  try {
    ...
    const chainSymbols = Array.from(new Set(wallets.map((w) => w.network.symbol as ChainSym)));
    const prices = await getUsdPrices(chainSymbols);

    // 🚀 OPTIMIZATION: Parallel processing with timeout protection
    const enriched = await Promise.all(
      wallets.map(async (w) => {
        const net = w.network.symbol as ChainSym;
        let native = 0;
        ...
        try {
          // 🚀 KEY CHANGE: Fetch native + USDT in parallel instead of sequentially
          if (net === "SOL") {
            const mint = USDT_ADDR.SOL;
            [native, usdt] = await Promise.all([
              withTimeout(getSolBalance(w.network.rpcUrl, w.address), 8000, 0),
              mint ? withTimeout(getSplTokenBalance(w.network.rpcUrl, w.address, mint), 8000, 0) : Promise.resolve(0),
            ]);
          } else if (net === "TRX") {
            const trc20 = USDT_ADDR.TRX;
            [native, usdt] = await Promise.all([
              withTimeout(getTrxBalance(w.network.rpcUrl, w.address), 8000, 0),
              trc20 ? withTimeout(getTrc20Balance(w.network.rpcUrl, w.address, trc20), 8000, 0) : Promise.resolve(0),
            ]);
          } else if (net === "ETH") {
            const erc20 = USDT_ADDR.ETH;
            [native, usdt] = await Promise.all([
              withTimeout(getEthBalance(w.address), 8000, 0),
              erc20 ? withTimeout(getErc20Balance(w.address, erc20), 8000, 0) : Promise.resolve(0),
            ]);
          } else if (net === "BTC") {
            native = await withTimeout(getBtcBalance(w.address), 10000, 0);
            usdt = 0;
          } else if (net === "DOGE") {
            native = await withTimeout(getDogeBalance(w.address), 10000, 0);
          } else if (net === "XRP") {
            native = await withTimeout(getXrpBalance(w.address), 8000, 0);
          } else if (net === "XMR") {
            try {
              const meta = w.meta ? JSON.parse(w.meta) : null;
              if (!meta?.walletName || !meta?.walletPassword) {
                ...
                native = await withTimeout(getXmrBalance(meta.walletName, meta.walletPassword), 10000, 0);
                console.log(`✅ ${meta.walletName} balance:`, native);
              }
            } catch (e: any) {
              console.error(`❌ XMR fetch failed for user ${userId}:`, e.message);
