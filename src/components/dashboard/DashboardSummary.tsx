...
export default function DashboardSummary() {
  ...
  useEffect(() => {
    ...
    loadMarket();
    const t = window.setInterval(loadMarket, 45_000);
    return () => {
      abort.abort();
      window.clearInterval(t);
    };
  ...
  const cards = useMemo(() => {
    const makeCard = (key: string, label: string, icon: React.ReactNode, color: string, path?: string) => {
      const native = getNative(key);
      const usdt = getUsdtOnChain(key);
      const combinedUsd = native.usd + usdt.usd;
      const value = fmtUSD(isNaN(combinedUsd) ? 0 : combinedUsd);
      const sub = Number(usdt.amount) > 0 ? `${native.amount} ${key}\n${usdt.amount} USDT` : `${native.amount} ${key}`;
      const m = market[key];
      const changePct = m?.pct ?? 0;
      const changeAbs = m?.abs ?? 0;
      const series = m?.spark && m.spark.length ? m.spark : [0, 0, 0, 0, 0, 0, 0];
      ...
      <div className={mounted ? "opacity-100 transition-opacity" : "opacity-0"}>
        {mounted && (
          <Swiper
            ...
            /** 🌐 RESPONSIVE SETTINGS */
            breakpoints={{
              320: { slidesPerView: 1.2, spaceBetween: 12 }, // small phones
              480: { slidesPerView: 2, spaceBetween: 14 },  // large phones
              640: { slidesPerView: 2.5, spaceBetween: 16 }, // tablets portrait
              768: { slidesPerView: 3, spaceBetween: 18 },   // small tablets landscape
