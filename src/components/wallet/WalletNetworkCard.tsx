...
import { SiSolana, SiEthereum, SiBitcoin, SiDogecoin, SiXrp, SiMonero } from "react-icons/si";
import Alert from "@/components/ui/alert/Alert";
import { WalletCardSkeleton } from "@/components/skeleton/SkeletonLoaders";
...
export function isTestnet(id: ChainIdLike): boolean {
  if (id == null) return PUBLIC_CHAIN_ENV !== "mainnet";
...
function getNetworkIcon(symbol: ChainSym, size: "sm" | "md" | "lg" = "md") {
  const sizeClass = size === "sm" ? "w-4 h-4" : size === "lg" ? "w-7 h-7" : "w-5 h-5 sm:w-6 sm:h-6";
  
  switch (symbol) {
    case "SOL":
      return <SiSolana className={`text-[#14F195] ${sizeClass}`} />;
    case "TRX":
      return <TronIcon className={sizeClass} />;
    case "ETH":
      return <SiEthereum className={`text-[#627EEA] ${sizeClass}`} />;
    case "BTC":
      return <SiBitcoin className={`text-[#F7931A] ${sizeClass}`} />;
    case "DOGE":
      return <SiDogecoin className={`text-[#C2A633] ${sizeClass}`} />;
    case "XRP":
      return <SiXrp className={`text-[#25A768] ${sizeClass}`} />;
    case "XMR":
      return <SiMonero className={`text-[#FF6600] ${sizeClass}`} />;
  }
}

/* ===============================================
   NATIVE AMOUNT SECTION - Responsive
=============================================== */
export type NativeAmountSectionProps = {
  symbol: ChainSym;
  tokenAmount: string;
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-baseline gap-1 sm:gap-2">
        <p className="text-lg sm:text-xl md:text-2xl font-bold text-gray-900 dark:text-gray-300">
          {tokenAmount} {symbol}
        </p>
...
/* ===============================================
   USDT AMOUNT SECTION - Responsive
=============================================== */
export type UsdtAmountSectionProps = {
  usdtTokenAmount?: string | null;
};
...
/* ===============================================
   ADDRESS SECTION - Fully Responsive
=============================================== */
export type AddressSectionProps = {
  symbol: ChainSym;
  address?: string;
...
export function AddressSection({
  ...
  const copyAddress = async () => {
    ...
    await navigator.clipboard.writeText(resolvedAddress);
    setAlertTitle('Copied Successfully');
    setAlertMessage("Address copied to clipboard!");
    setAlertVariant('success');
    setAlertVisible(true);
  };
  ...
  const textSizeClass = 
    textSize === "xl" ? "text-base sm:text-lg md:text-xl" :
    textSize === "lg" ? "text-sm sm:text-base md:text-lg" :
    textSize === "base" ? "text-xs sm:text-sm md:text-base" :
    textSize === "sm" ? "text-[10px] sm:text-xs md:text-sm" :
  ...
  return (
    <div className="mb-0 mt-2">
      ...
      {/* Address Row - Fully Responsive */}
      <div className={`flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-0
                       text-gray-600 dark:text-gray-300 ${textSizeClass} ${className ?? ""}`}>
        
        {/* Left: Address */}
        <div className="flex flex-col gap-1 min-w-0 flex-1">
          <p className={`truncate ${addressTextClassName ?? "text-brand-500 dark:text-brand-400"}`}>
        ...
        <div className="flex items-center gap-3 sm:gap-4 md:gap-6 flex-shrink-0">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <button
              ...
              disabled={!address}
            >
              {showAddress ? <FaEyeSlash className="w-3 h-3 sm:w-4 sm:h-4" /> : <FaEye className="w-3 h-3 sm:w-4 sm:h-4" />}
            </button>
          </div>
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className={`flex items-center gap-1 text-brand-500 dark:text-brand-400 hover:underline
...
/* ===============================================
   WALLET NETWORK CARD - Fully Responsive
=============================================== */
export type WalletNetworkCardProps = {
  name: string;
  symbol: ChainSym;
...
export default function WalletNetworkCard({
  ...
  const CardInner = (
    <div className="flex flex-col group h-full rounded-2xl border border-gray-200/60 dark:border-white/10
                    bg-white/70 dark:bg-[#0B1220]/80 backdrop-blur-sm min-h-[200px] sm:min-h-[223px]
                    p-4 sm:p-5 shadow-sm transition-all duration-300 hover:shadow-md hover:-translate-y-0.5">
      
      {/* Header - Responsive */}
      <div className="flex items-start justify-between mb-3 sm:mb-4">
        <div className="flex items-center gap-2 sm:gap-3">
          {/* Icon */}
          <div className="h-9 w-9 sm:h-10 sm:w-10 md:h-11 md:w-11 rounded-lg sm:rounded-xl 
                          bg-gray-100 dark:bg-white/10
                          ring-1 ring-inset ring-black/5 dark:ring-white/10
        ...
        {/* Network Badge - Optional, hidden on small screens */}
        {/* <span className="hidden sm:inline-block text-[10px] px-2 py-0.5 rounded-full tabular-nums
                          bg-gray-100 dark:bg-white/10
                          ${onTestnet ? "text-amber-600 dark:text-amber-400" : "text-gray-600 dark:text-gray-300"}"
              aria-label={networkLabel}
              title={networkLabel}>
      ...
      {/* Amounts - Responsive */}
      <div className="h-full flex items-center">
        <div className="mt-2 sm:mt-3 md:mt-4 mb-2 sm:mb-3 md:mb-4 w-full">
          <div className="flex items-end justify-between">
      ...
      {/* Divider + Address - Responsive */}
      <div className="mt-auto pt-2 sm:pt-3 border-t border-gray-200/60 dark:border-white/10">
  ...
  return hairline ? (
    <div className={`h-full rounded-2xl p-px bg-gradient-to-r ${hairlineGradient}`}>
      {CardInner}
    </div>
  ) : (
    CardInner
