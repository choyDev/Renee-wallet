...
const getTokenIcon = (token: string) => {
  ...
  if (token.startsWith("ETH"))
    return <FaEthereum className="text-[#627EEA] w-5 h-5" />;
  if (token.startsWith("BTC"))
    return <FaBitcoin className="text-[#F7931A] w-5 h-5" />;
  if (token.startsWith("DOGE"))
    return <SiDogecoin className="text-[#C2A633] w-5 h-5" />;
  if (token.startsWith("XMR"))
    return <SiMonero className="text-[#FF6600] w-5 h-5" />;
  if (token.startsWith("XRP"))
    return <SiXrp className="text-[#0A74E6] w-5 h-5" />;
  return null;
};
...
export default function BridgeModal({
  ...
  const initialFrom = NETWORKS.find(
    (n) => n.symbol === currentChain
  )!;
  const [fromNetwork] = useState(initialFrom);
  const [fromToken, setFromToken] = useState(
    initialFrom.tokens[0]
  );
  const [toNetwork, setToNetwork] = useState(
    NETWORKS.find((n) => n.symbol !== currentChain)!
  );
  const [toToken, setToToken] = useState(toNetwork.tokens[0]);
  const [amount, setAmount] = useState("");
  const [quote, setQuote] = useState<any>(null);
  ...
  /* ---------- Reset to-token when network changes ---------- */
  useEffect(() => {
    setToToken(toNetwork.tokens[0]);
  }, [toNetwork]);
  ...
  const isCompatible = useMemo(() => {
    const fromIsBTC =
      fromToken.startsWith("BTC") ||
      fromNetwork.symbol === "BTC";
    const toIsBTC =
      toToken.startsWith("BTC") ||
      toNetwork.symbol === "BTC";

    const fromIsUSDT = fromToken.includes("USDT");
    const toIsUSDT = toToken.includes("USDT");

    if ((fromIsBTC && toIsUSDT) || (toIsBTC && fromIsUSDT))
      return false;
        ...
        >
          ...
          >
            ...
            <button
              onClick={() => !submitting && onClose()}
              disabled={submitting}
              className="absolute right-4 top-4 text-gray-800 dark:text-gray-300  transition disabled:opacity-40"
            >
              <FiX size={22} />
            </button>
            ...
            <div>
              ...
              <div className="flex items-center bg-white/5 border border-white/10 rounded-xl px-4 py-3 mt-1 text-gray-800 dark:text-gray-300">
                <fromNetwork.Icon
                  color={fromNetwork.color}
                  className="w-5 h-5"
                />
                <span className="ml-2">{fromNetwork.name}</span>
              </div>

              <label className="text-sm text-gray-800 dark:text-gray-300 mt-2">
                From Token
              </label>
              ...
              {fromNetwork.tokens.length > 1 ? (
                ...
                <div className="flex items-center gap-2 bg-white/5 border border-white/10 rounded-xl px-4 py-3 mt-2 text-gray-800 dark:text-gray-300">
                  {getTokenIcon(fromNetwork.tokens[0])}
                  <span>{fromNetwork.tokens[0]}</span>
                </div>
              )}
            ...
            <div>
              ...
              <input
                ...
                value={amount}
                onChange={(e) => setAmount(e.target.value)}
                className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 mt-1 text-gray-800 dark:text-gray-300  placeholder-gray-500 focus:outline-none"
              />
            </div>
            ...
            {!isCompatible ? (
              ...
              quote && (
                <motion.div
                  initial={{ opacity: 0, y: 10 }}
                  animate={{ opacity: 1, y: 0 }}
                  className="rounded-xl bg-gradient-to-r from-blue-500/10 to-indigo-500/10 border border-blue-500/30 px-4 py-3 flex items-center justify-between text-gray-800 dark:text-gray-300 mt-2"
                >
                  <div className="flex items-center gap-2">
                    {getTokenIcon(fromToken)}
                    <span className="font-semibold">
...
function TokenDropdown({
  ...
  return (
    <div className="relative">
      <button
        onClick={() => setOpen(!open)}
        className="flex items-center justify-between w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 mt-2 text-gray-800 dark:text-gray-300 hover:bg-white/10 transition"
      >
        <div className="flex items-center gap-2">
          {getTokenIcon(current)}
          <span>{current}</span>
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -5 }}
            className="absolute mt-2 w-full border-gray-900 
                      dark:border-gray-900 
                      bg-gray-50 
                      dark:bg-[#1A1730] rounded-xl shadow-lg z-20 overflow-hidden"
          >
            {tokens.map((t: string) => (
              <button
                key={t}
                ...
                className="flex items-center gap-2 w-full px-4 py-2 text-left text-gray-800 dark:text-gray-300 hover:bg-blue-500/20 transition"
              >
                {getTokenIcon(t)}
                <span>{t}</span>
              </button>
...
function Dropdown({
  ...
  return (
    <div className="relative">
      <button
        onClick={() => setOpen(!open)}
        className="flex items-center justify-between w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 mt-1 text-gray-800 dark:text-gray-300 hover:bg-white/10 transition"
      >
        <div className="flex items-center gap-2">
          <current.Icon
            color={current.color}
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -5 }}
            className="absolute mt-2 w-full dark:bg-[#1A1730] border border-white/10 rounded-xl shadow-lg z-20 overflow-hidden"
          >
            {items.map((item: any) => (
              <button
                key={item.symbol}
